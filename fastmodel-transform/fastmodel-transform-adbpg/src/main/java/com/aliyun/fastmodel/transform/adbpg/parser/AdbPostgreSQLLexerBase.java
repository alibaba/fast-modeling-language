 ...
 * @author panguanjing
 */
public abstract class AdbPostgreSQLLexerBase extends Lexer {
    protected final Deque<String> tags = new ArrayDeque<>();

    protected AdbPostgreSQLLexerBase(CharStream input) {
        super(input);
    ...
    public void HandleNumericFail() {
        ...
        setType(AdbPostgreSQLLexer.Integral);
    }

    public void HandleLessLessGreaterGreater() {
        if (getText() == "<<") {setType(AdbPostgreSQLLexer.LESS_LESS);}
        if (getText() == ">>") {setType(AdbPostgreSQLLexer.GREATER_GREATER);}
    }
