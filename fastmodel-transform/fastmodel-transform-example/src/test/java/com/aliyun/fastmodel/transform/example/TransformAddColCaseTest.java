 ...
 * @date 2021/2/24
 */
public class TransformAddColCaseTest extends BaseTransformCaseTest {
    ...
    @Test
    public void testAddNewColWithPrimary() {
    ...
    public void testAddNewFiledNotNull() {
        ...
        List<ColumnDefinition> columns =
            ImmutableList.of(
                ColumnDefinition.builder().colName(new Identifier("c1")).dataType(new GenericDataType(
                    new Identifier(DataTypeEnums.BIGINT.name()))).primary(true).comment(new Comment("comment")).build()
            );
        after = CreateDimTable.builder().tableName(QualifiedName.of("a.dim_shop")).columns(columns).build();
    }
}
