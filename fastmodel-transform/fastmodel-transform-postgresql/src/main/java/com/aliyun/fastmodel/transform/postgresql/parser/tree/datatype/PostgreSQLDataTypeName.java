 ...
 * @author panguanjing
 */
public enum PostgreSQLDataTypeName implements ISimpleDataTypeName {
    /**
     * integer
     */
    ...
    PostgreSQLDataTypeName(String value, String alias, Dimension dimension, SimpleDataTypeName simpleDataTypeName) {
        this.value = value;
        this.alias = alias;
     ...
     *
     * @param value 值或者别名
     * @return {@link PostgreSQLDataTypeName}
     */
    public static ISimpleDataTypeName getByValue(String value) {
        int index = value.indexOf(PostgreSQLArrayDataTypeName.VALUE_SUFFIX);
        if (index > 0) {
            return PostgreSQLArrayDataTypeName.getByValue(value);
        }
        PostgreSQLDataTypeName[] hologresDataTypeNames = PostgreSQLDataTypeName.values();
        //先从value中获取
        for (PostgreSQLDataTypeName hologresGenericDataType : hologresDataTypeNames) {
