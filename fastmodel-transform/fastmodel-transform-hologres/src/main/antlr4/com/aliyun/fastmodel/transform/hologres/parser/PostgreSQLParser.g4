...
createTable
    : CREATE (GLOBAL | LOCAL)? TEMPORARY? UNLOGGED? TABLE ifNotExists?
       tableName
       (
         LIKE tableName
...
columnConstraint
    ...
    | CONSTRAINT uid                                                #constraintColumnConstraint
    | GENERATED ((BY DEFAULT) | ALWAYS)  AS  IDENTITY               #generatedAlwaysConstraint
    | CHECK expression                                              #checkConstraint
    ;

indexParameters
    :   (INCLUDE indexColumnNames)
        | (WITH  expression )
        | (USING INDEX TABLESPACE uid)
    ;
...
tableConstraint
    ...
    | (CONSTRAINT name=uid?)?
      ...
      indexType? indexColumnNames indexOption*  indexParameters?                  #uniqueKeyTableConstraint
    | (CONSTRAINT name=uid?)?
...
partitionFunctionDefinition
    ...
    | RANGE ( '(' expression ')' | COLUMNS '(' uidList ')' | functionArgs )        #partitionFunctionRange
...
selectStatement
    : withClause? querySpecification lockClause?                                #simpleSelect
    | withClause? queryExpression lockClause?                                   #parenthesisSelect
...
withClause:
    WITH RECURSIVE? withExpr (COMMA withExpr)*
    ;

withExpr:
    ((tableName (LR_BRACKET selectElements RR_BRACKET)?) | (tableName selectElements))
    AS LR_BRACKET (selectStatement|updateStatement|deleteStatement|insertStatement) RR_BRACKET
;
...
selectElement
    ...
    | ARRAY MLR_BRACKET uid (COMMA uid)* MRR_BRACKET #selectExpressionElement
    ;
...
fromClause
    : FROM ( tableSources | functionCall)
...
dataType
    ...
    | typeName=(
        TINYINT | SMALLINT | MEDIUMINT | INT | INTEGER | BIGINT | BIGSERIAL
      )
    ...
    | typeName=INTERVAL lengthOneDimension?
      (
           | YEAR
           | MONTH
           | DAY
