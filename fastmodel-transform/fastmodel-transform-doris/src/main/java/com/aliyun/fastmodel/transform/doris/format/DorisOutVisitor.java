 ...
 * @date 2023/9/5
 */
public class DorisOutVisitor extends FastModelVisitor implements DorisAstVisitor<Boolean, Integer> {

    public static final String MAXVALUE = "MAXVALUE";
    private final DorisContext context;

    public DorisOutVisitor(DorisContext context) {
    ...
    protected String formatProperty(List<Property> properties) {
        List<Property> collect = properties.stream().filter(
            p -> {
                PropertyKey byValue = DorisPropertyKey.getByValue(p.getName());
    ...
    protected String formatStringLiteral(String s) {
        return new DorisExpressionVisitor(context).formatStringLiteral(s);
    }
    ...
    protected String formatExpression(BaseExpression baseExpression) {
        return new DorisExpressionVisitor(context).process(baseExpression);
    }
    ...
    public Boolean visitExpressionPartitionedBy(ExpressionPartitionBy expressionPartitionedBy, Integer indent) {
        ...
        if (expressionPartitionedBy.getFunctionCall() == null) {
            ...
            String function = formatExpression(functionCall);
    ...
    public Boolean visitMultiRangePartition(MultiRangePartition multiRangePartition, Integer context) {
        ...
        builder.append(")");
        builder.append(" ").append(formatExpression(multiRangePartition.getIntervalLiteral()));
        return true;
    }
