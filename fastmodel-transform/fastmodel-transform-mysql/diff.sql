...
ALTER TABLE model_table ADD
(
   `storage_policy_code` VARCHAR(64) COMMENT '存储策略编码'
);
ALTER TABLE online_dqc_rule ADD
(
   `model_code` VARCHAR(128) COMMENT '模型code'
);
CREATE INDEX idx_time_period_name ON `time_period` (`name`);
ALTER TABLE t_online_model CHANGE COLUMN `business_process_name` `business_process_name` VARCHAR(70) NULL;
ALTER TABLE t_online_model ADD
(
   `source_project_id` VARCHAR(128) COMMENT '源项目id',
   `model_code`        VARCHAR(128) COMMENT '模型code'
);
ALTER TABLE t_release_package_object CHANGE COLUMN `need_realease_fml` `need_realease_fml` mediumtext;
ALTER TABLE t_release_package_object CHANGE COLUMN `big_engine_dsl` `big_engine_dsl` mediumtext;
ALTER TABLE t_release_package_object ADD
(
   `release_object_type` VARCHAR(64) COMMENT '部署对象类型',
   `source_project_id`   VARCHAR(128) COMMENT '源项目id'
);
ALTER TABLE t_vc_commit_object CHANGE COLUMN `business_process_name` `business_process_name` VARCHAR(70) NULL;
ALTER TABLE t_vc_commit_object ADD
(
