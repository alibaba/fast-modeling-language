 ...
 * @date 2022/6/10
 */
public class HiveCodeGeneratorModelTest {
    ...
    public void testConvertToTableIssueLifecycle() {
        ...
        DdlGeneratorModelRequest request1 = DdlGeneratorModelRequest.builder()
            .config(TableConfig.builder().dialectMeta(DialectMeta.DEFAULT_HIVE).build())
    ...
    @Test
    public void testGeneratorUnionType() {
        String text = getText("union_type.txt");
        HiveTransformer hiveTransformer = new HiveTransformer();
        BaseStatement reverse = hiveTransformer.reverse(new DialectNode(text));
        Table table = hiveTransformer.transformTable(reverse, TransformContext.builder().build());
        Node node = hiveTransformer.reverseTable(table);
        DialectNode transform = hiveTransformer.transform((BaseStatement)node);
        assertEquals("CREATE TABLE union_test\n"
            + "(\n"
    ...
    private String getText(String file) {
        return IOUtils.resourceToString("/hive/" + file, Charset.defaultCharset());
    }
}
